            ...
            <div class="delete-form">
              <ul>
                ...
                <li class="d-flex justify-content-between align-items-center bt-line">
                  <div class="left-col title text-primary"><a href="{{route('adgetEditNews', ['id' => $item->id])}}">{{$item->title}}</a></div>
                  <div class="right-col text-right"><a class="btn delete-btn" href="{{route('adgetDelNews', ['id' => $item->id])}}" onclick="return confirm('Bạn có chắc chắn xóa bài viết này?');"><i class="icon icon-close"></i></a></div>
                </li>
                @endforeach
              </ul>
            </div>
